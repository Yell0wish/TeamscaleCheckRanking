...
log_file_path = Path(config["output_dir"]) / "create_pd_dataset.log"
...
def create_dataset(release, abs_semgrep_path, abs_labels_path, abs_rules_path, abs_repo_path=None):
    """
    ...
    - 读取 abs_rules_path 获取违反规则的 rule_id 和 severity
    - 读取 abs_semgrep_path 获取实际违反规则情况
    """
    # 读取缺陷代码行
    labels_df = []
    ...
    # 读取finding_type
    rules_df = pd.read_csv(abs_rules_path)
    ...
    # 读取实际违反规则情况
    semgrep_df = pd.read_csv(abs_semgrep_path)
    logging.info(f"读取 {len(semgrep_df)} 条违反规则记录")

    semgrep_dict = {}
    # location为文件相对路径，line为行号
    # 遍历数据集
    for _, row in semgrep_df.iterrows():
        try:
            finding_type = row["check_id"].strip()
            location = row["path"].strip()
            ...
            logging.info(f'读取行号失败: {e}，未预期错误')
        
        # 遍历 start_line 到 end_line 之间的每一行
        for line in range(start_line, end_line + 1):
            semgrep_dict.setdefault((location, line), set()).add(finding_type)
            if finding_type not in finding_mapping:
                logging.info(f"未在规则列表中找到违反规则类型: {finding_type}")
    


    logging.info(f"读取 {len(semgrep_dict)} 行违反规则记录")
    ...
    # 格式 file, line, code snippet, label, rules(130条)
    dataset = []
    ...
    for file_idx, rel_path in enumerate(src_files):
        for line_no, code_line in enumerate(texts_lines[file_idx], start=1):
            ...
            # 规则特征
            if (rel_path, line_no) in semgrep_dict:
                for rule in semgrep_dict[(rel_path, line_no)]:
                    data_item[finding_mapping[rule]] = 1
                    temp_count += 1
...
ABS_RULES_PATH = Path(config["output_dir"]) / "pmd_rules.csv"

def main():
    start_time = time.time()  # 记录总任务的开始时间

    output_dir = Path(config["output_dir"]) / "semgrep_dataset"
    ...
    for project in DATASET_PROJECT_MAP.keys():
        ...
        for version in version_list:
            abs_semgrep_path = Path(config["output_dir"]) / f'processed_data/{version}-semgrep.csv'
            abs_labels_path = Path(config['line_level_bug_labels_dir']) / f"{version}_defective_lines_dataset.csv"

            dataset = create_dataset(version, abs_semgrep_path, abs_labels_path, ABS_RULES_PATH)
            

            assert(len(dataset[0]) == 4+130)

            df_ = pd.DataFrame(dataset, columns=["file", "line", "code snippet", "label"] + [f"feature_{i}" for i in range(130)])

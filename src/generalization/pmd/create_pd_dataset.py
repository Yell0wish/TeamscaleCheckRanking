...
def create_dataset(release, abs_pmd_path, abs_labels_path, abs_rules_path, abs_repo_path=None):
    """
    ...
    # 读取实际违反规则情况
    pmd_df = pd.read_csv(abs_pmd_path)
    logging.info(f"读取 {len(pmd_df)} 条违反规则记录")
    pmd_dict = {}
    # location为文件相对路径，line为行号
    # 遍历数据集
    for _, row in pmd_df.iterrows():
        try:
            finding_type = row["ruleID"].strip()
            location = row["location"].strip()
            start_line = int(float(str(row["startLine"]).strip()))
            end_line = int(float(str(row["endLine"]).strip()))
        ...
        for line in range(start_line, end_line + 1):
            pmd_dict.setdefault((location, line), set()).add(finding_type)
            if finding_type not in finding_mapping:
                logging.info(f"未在规则列表中找到违反规则类型: {finding_type}")


    logging.info(f"读取 {len(pmd_dict)} 行违反规则记录")
    ...
    # 格式 file, line, code snippet, label, rules(271条)
    dataset = []
    ...
    for file_idx, rel_path in enumerate(src_files):
        for line_no, code_line in enumerate(texts_lines[file_idx], start=1):
            ...
            # 规则特征
            if (rel_path, line_no) in pmd_dict:
                for rule in pmd_dict[(rel_path, line_no)]:
...
def main():
    ...
    output_dir = Path(config["output_dir"]) / "pmd_dataset"
    ...
    for project in DATASET_PROJECT_MAP.keys():
        ...
        for version in version_list:
            abs_pmd_path = Path(config["output_dir"]) / f'processed_data/{version}-pmd.csv'
            abs_labels_path = Path(config['line_level_bug_labels_dir']) / f"{version}_defective_lines_dataset.csv"

            dataset = create_dataset(version, abs_pmd_path, abs_labels_path, ABS_RULES_PATH)
            assert(len(dataset[0]) == 4+271)

            df_ = pd.DataFrame(dataset, columns=["file", "line", "code snippet", "label"] + [f"feature_{i}" for i in range(271)])

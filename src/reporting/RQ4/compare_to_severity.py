...
def log_init():
    log_file_path = RESULTS_DIR / "compare_to_severity.log"
    logging.basicConfig(
        level=logging.INFO,
        format="%(message)s",
        handlers=[
...
def reorder(input_path: str):
    # 读取 CSV
    df = pd.read_csv(input_path)
    
    # 检查必要列是否存在
    required_cols = {'id', 'typeid', 'severity'}
    if not required_cols.issubset(df.columns):
        raise ValueError(f"输入文件必须包含列: {required_cols}")
    
    # 按 severity 排序：RED 在前，YELLOW 在后
    df_sorted = pd.concat([
        df[df['severity'] == 'RED'],
        df[df['severity'] == 'YELLOW']
    ], ignore_index=True)
    
    # 只保留 id 列并改名为 check_id
    result = df_sorted[['id']].rename(columns={'id': 'check_id'})
    return result
...
def rank_dataset_by_checks(dataset_df: pd.DataFrame, ranked_checks: list):
    """
    根据 ranked_checks 对 dataset 排序:
    - 命中 ranked_checks 的行排在前面，未命中在后面
    - 多条命中时，取“最佳名次”(rank最小)作为主键
    - 同一组内保持稳定顺序
    """
    df = dataset_df.copy()
    ...
    # 遍历 ranked_checks，找到每行命中的最优顺序
    for rank, fid in enumerate(ranked_checks):
        fid = f'feature_{fid}'
        if fid not in df.columns:
            print(f"Warning: feature '{fid}' not found in dataset columns.")
            continue
        hit_mask = df[fid] > 0
        df.loc[hit_mask, '_has_hit'] = True
    ...
    df_sorted = df.sort_values(
        by=['_has_hit', '_best_rank', '_orig_idx'],
        ascending=[False, True, True],
        kind='mergesort'
    ).drop(columns=['_orig_idx', '_has_hit', '_best_rank'])
...
def eval_with_ranked_checks(versions, version_paths, ranked_checks, output_path):
    ...
    for i in range(len(versions)):
        ...
        # —— 按 ranked_checks 排序
        ranked_df  = rank_dataset_by_checks(data_df, ranked_checks)
...
def main():
    ...
    ranked_df = reorder(config["checks_index_color"])
    ...
    for idx, project in enumerate(DATASET_PROJECT_MAP.keys()):
        ...
        version_paths = [
            # rf"F:\FilesForResearch\codeqlResearchProject\TeamScaleAnalysis\research\glance\final_dataset_250812\{project}-{v}.parquet"
            Path(config["dataset_dir"]) / f"{project}-{v}.parquet"
            for v in versions
        ]
    ...
    # —— 保存总表
    out_csv = RESULTS_DIR / 'compare_to_severity.csv'
